...
/** Sections that live in the narrow right column; everything else goes in the main column. */
const ASIDE: ReadonlySet<SectionId> = new Set<SectionId>(["skills", "education", "languages"]);
              ...
              <article key={entry.id} className="break-inside-avoid">
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="font-bold">
                    {entry.role || "Role"}
                    {entry.company && <span className="font-normal text-zinc-600"> &mdash; {entry.company}</span>}
                  </h3>
  ...
  return (
    <div className="font-sans text-[9pt] leading-[1.35] text-zinc-900">
      <header className="flex items-end justify-between gap-6 border-b-2 border-zinc-800 pb-2 break-after-avoid">
        <div>
          <h1 className="text-[17pt] font-bold leading-none tracking-tight">{data.fullName || "Your Name"}</h1>
      ...
      <div className="mt-3 grid grid-cols-[minmax(0,1fr)_52mm] gap-x-6">
        <main className="min-w-0 space-y-3.5">
          {order.filter((id) => !ASIDE.has(id)).map((id) => (
