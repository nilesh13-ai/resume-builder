        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
          {data.education.map((e) => (
            <div key={e.id} className="break-inside-avoid">
              ...
              <p className="text-[9.5pt] text-zinc-600">{[e.institution, e.year].filter(Boolean).join(", ")}</p>
            </div>
          ))}
        </div>
      </Section>
    ...
    skills: (
      <Section title="Skills">
        <ul className="flex flex-wrap gap-1.5 break-inside-avoid">
          {data.skills.map((s) => (
            <li key={s} className="rounded-full border-2 border-zinc-900 px-2.5 py-0.5 text-[9pt] font-semibold text-zinc-900">
              {s}
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="font-sans text-[10pt] leading-[1.45] text-zinc-800">
      <header className="rounded-lg bg-zinc-900 px-8 py-7 text-white break-after-avoid">
        <h1 className="text-[30pt] font-black uppercase leading-none tracking-tight">{data.fullName || "Your Name"}</h1>
        {data.jobTitle && <p className="mt-2 text-[13pt] font-semibold text-rose-400">{data.jobTitle}</p>}
        {contact.length > 0 && (
