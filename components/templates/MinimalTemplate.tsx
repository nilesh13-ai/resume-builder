...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mt-4 border-t border-zinc-200 pt-3">
      <h2 className="mb-2.5 text-[8.5pt] font-medium uppercase tracking-[0.25em] text-zinc-400 break-after-avoid">{title}</h2>
      {children}
    </section>
  );
}
  ...
  return (
    <div className="font-sans text-[9.5pt] leading-[1.5] text-zinc-800">
      <header className="break-after-avoid">
        <h1 className="text-[21pt] font-light leading-tight tracking-tight text-zinc-900">{data.fullName || "Your Name"}</h1>
